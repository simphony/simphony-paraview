...
plugin = ['paraview_tools = simphony_paraview.plugin']
...
def write_version_py(filename='simphony_paraview/_version.py'):
    template = """\
# THIS FILE IS GENERATED FROM SIMPHONY-PARAVIEW SETUP.PY
version = '{version}'
...
"""
    ...
    # write_version_py(), otherwise the import of simphony_paraview._version
    # messes up the build under Python 3.
    fullversion = VERSION
    if os.path.exists('.git'):
        git_rev, dev_num = git_version()
    elif os.path.exists('simphony_paraview/_version.py'):
        # must be a source distribution, use existing version file
        try:
            from simphony_paraview._version import git_revision as git_rev
            from simphony_paraview._version import full_version as full_v
        except ImportError:
            raise ImportError("Unable to import git_revision. Try removing "
                              "simphony_paraview/_version.py and the build "
